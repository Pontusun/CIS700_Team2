...
*******************************/
  ...
  bool equalCoordinates(std::vector<double> in1, std::vector<double> in2){
...
*/
  ...
  void gotoIdle(void){
    ...
    if(this->userInput.compare(" ") != 0){
      // parse input 
      std::istringstream iss(this->userInput);
      iss >> cmd;
      this->userInput = " ";
      if(cmd.compare("cmd_idle") == 0){
        this->fsm = IDLE;
      }
    }
  ...
  void spin_findObject(){
      ...
      switch(fsm){
        case IDLE:
          if(this->userInput.compare(" ") != 0){
            ...
            this->userInput = " ";
            iss >> cmd;
            if(cmd.compare("cmd_goto") == 0)
            {
                iss >> location;
                iss >> object;
                navGoal = this->coordinates[location];
                homeBase = this->coordinates["grasp_lab"];
                this->objectTargets.push_back(object);
                this->fsm = GOTO;
            }
            else if(cmd.compare("cmd_follow") == 0){
              this->fsm = EXPLORE_FACE;
            }
            else if(cmd.compare("cmd_teleop") == 0){
        ...
        case GOTO : 
          // gotoIdle();
         words.data = "Goto the destination!";    
    stringPub.publish(words);
	 ...
	 result = goal(navGoal);
         ...
         if (result){
            ROS_INFO("Wiggles has reached navGoal");
           
   	 words.data = "Destination reached!";    
   	 stringPub.publish(words);
    	 this->fsm = EXPLORE_OBJ;
          }
          break;
    ...
    words.data = "Looking for object!";    
    stringPub.publish(words);
          if(detect_obj == true){
                ROS_INFO("Wiggles has reached navGoal");
           
   	 words.data = "Object found!";    
   	 this->fsm = WAIT_OBJ;
            break;
          }
        ...
        case WAIT_OBJ :
          ...
          //publish reply
          msg_feedback.data = "location: success object: success";

          androidPub.publish(msg_feedback);
          navGoal = homeBase;
          this->fsm = GOTO_BASE;
          break;
    ...
    result = goal(navGoal);
        ...
        case EXPLORE_FACE : 
          gotoIdle();
          if( detect_face == true){
            this->fsm = FOLLOW_FACE;
            words.data = "I see a face, start following";
            stringPub.publish(words);
            break;
          }
          this->navigationExplore();
...
protected:
  ...
  // controller members
  enum FSM {IDLE, GOTO, GOTO_BASE, EXPLORE_OBJ, WAIT_OBJ, EXPLORE_FACE, FOLLOW_FACE};
  FSM fsm;
  bool detect_obj;
  bool detect_face;
